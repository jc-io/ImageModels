// src/pages/ImageGenPage.js
import { React, useState, useEffect } from "react";
import axios from "axios";
import { toast } from "react-toastify";

const ImageGenPage = () => {
  // Main settings state vars
  ...
  /**
   * Sets image generation model to Stable Diffusion XL (High Detail) and resets prompt input box
  ...
  /**
   * Toggles the visibility and usability of the Image Settings dropdown menu
  ...
  /**
   * Sends request to generate the image(s) based on user specified parameters, then stores in images array
   * @function handleGen
   */
  const handleGen = async () => {
    ...
    if (prompt.length > 0 && postCount < 6) {//check model chosen
      ...
      try {
        // Send the same image generation request numImage times with specified parameters
        for (let i = 0; i < numImages; i++) {
          // Forms request data to send to backend server (main.py)
          ...
          // Sends the request to the server env
          const response = await axios.post(
            `${process.env.REACT_APP_BACKEND_URL}/generate`,
            formData
          );

          // Once the data is sent back, unpack and display images as it receives them
          const data = response.data;
          setImages(prevImages => [...prevImages, ...data.images.map(image => image.image_data)]);
          setPageState('result');
          setPostCount(prevCount => prevCount + 1);
          toast.dismiss()
          toast.success(`Success: Image ${i+1} Generated!`, { autoClose: 5000});
          if (numImages != i+1){
  ...
  /**
   * Archives or saves the featured image in to the user's archive page
  ...
  const archiveImage = async () => {
    ...
    formData.append(
      "image",
      featuredImage ? featuredImage : images.length > 0 ? images[0] : ""
    );
    formData.append("prompt", prompt);
    formData.append("model", selectedModel);
    ...
    // Sends the request to the server env
    axios
      .post(`${process.env.REACT_APP_BACKEND_URL}/Archive`, formData, {
        headers: {
  ...
  /**
   * Changes selected image to the featured image and swaps former featured image to selectable images
  ...
  /**
   * Resets state vars back to empty and returns to main state
   * @function goBack
   */
  const goBack = () => {
  ...
  /**
   * Allows users to download the image to their local PC
   * @function share
   */
  const share = () => {
        ...
        <div className='scrollable-container'>
          ...
          {pageState === "main" && (
            <div className='container py-10 px-10 mx-0 min-w-full flex flex-col items-center'>
              ...
              <textarea
                id='prompt-input'
                className='bg-gray-50 border border-gray-300 text-gray-900 rounded-lg focus:ring-blue-500 focus:border-blue-500 block p-2.5 dark:bg-gray-700
                  dark:border-gray-600 dark:placeholder-gray-400 dark:text-white dark:focus:ring-blue-500 dark:focus:border-blue-500 w-2/3 h-24'
              ...
              <div className='relative inline-block text-left'>
                <button
                  ...
                  type='button'
                  className='inline-flex justify-between w-[27.4rem] rounded-md border border-gray-300 shadow-sm px-4 py-2 bg-gray-200 text-base
                    font-medium text-gray-700 hover:bg-gray-300 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 focus:ring-offset-gray-100'
                >
              ...
              <br />
              <button
                className='bg-darker hover:bg-buttonHover text-white font-bold py-2 px-4 rounded items-center'
                onClick={handleGen}
              >
                Generate Image
              </button>
            ...
            <div className='container py-10 px-10 mx-0 min-w-full flex flex-col items-center'>
              ...
              <div className='text-center'>
                <div role='status'>
                  ...
                  >
                    <path
                      d='M100 50.5908C100 78.2051 77.6142 100.591 50 100.591C22.3858 100.591 0 78.2051 0 50.5908C0 22.9766 22.3858 0.59082 50 0.59082C77.6142 0.59082 100 22.9766 100
                    ...
                    <path
                      d='M93.9676 39.0409C96.393 38.4038 97.8624 35.9116 97.0079 33.5539C95.2932 28.8227 92.871 24.3692 89.8167 20.348C85.8452 15.1192 80.8826 10.7238 75.2124
                        7.41289C69.5422 4.10194 63.2754 1.94025 56.7698 1.05124C51.7666 0.367541 46.6976 0.446843 41.7345 1.27873C39.2613 1.69328 37.813 4.19778 38.4501 6.62326C39.0873
                        9.04874 41.5694 10.4717 44.0505 10.1071C47.8511 9.54855 51.7191 9.52689 55.5402 10.0491C60.8642 10.7766 65.9928 12.5457 70.6331 15.2552C75.2735 17.9648 79.3347
            ...
            <>
              <div className='image-display text-center flex flex-col items-center overflow-hidden'>
                ...
                <div className='w-full max-w-[57vh]'>
                  {images.length > 0 && (
                    <img
                      className='h-auto w-full rounded-lg mb-1'
                      src={featuredImage}
                      alt='Featured Image'
                    />
                  )}
                </div>
                {/* Gallery Selectable Area */}
                {numImages > 1 && (
                  <h3 className='text-white font-bold mb-1'>
                    Generated Selectable Images:
                  </h3>
                )}
              ...
              <div className='flex justify-center space-x-28 mt-4'>
                ...
                <button
                  className='inline-flex items-center justify-center bg-blue-500 hover:bg-blue-400 text-white font-bold py-2 w-48 border-b-4 border-blue-700 hover:border-blue-500 rounded'
